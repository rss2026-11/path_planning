...
class PurePursuit(Node):
    ...
    def pose_callback(self, odometry_msg):
        ...
        # self.get_logger().info(f"Odometry message has x: {x} & y: {y}")
    ...
    def find_nearest_point_on_trajectory(self, pos):
        ...
        # self.get_logger().info(f"Points seen on trajectory: {points}")

        # self.get_logger().info(f"Car position: {pos}")
    ...
    def find_lookahead_point(self, car_pos, segment_idx):
        ...
        for i in range(segment_idx, len(points) - 1):
            ...
            if intersection is not None:
                self.get_logger().info(f"Found intersection at : {intersection}")
                return intersection
        return points[-1]
